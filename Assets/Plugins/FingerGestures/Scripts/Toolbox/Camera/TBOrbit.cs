...
//        pitch = IdealPitch = angles.x;
...

	{
		...
		distance = IdealDistance = Vector3.Distance(target.position, transform.position);
		yaw = IdealYaw = angles.y;
		pitch = IdealPitch = angles.x;
		ResetPanning();
	}
...
//		}
        ...
        if( collisionLayerMask != 0 )
        {
            ...
            RaycastHit hit;
			if( Physics.Raycast( desiredPos, dir, out hit, dist, collisionLayerMask ) )
            {
				desiredPos = hit.point - dir * 0.01f;
